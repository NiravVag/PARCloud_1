...
{
    public class ResetInterfaceEventHandler : IRequestHandler<ResetInterfaceEventCommand, ResetInterfaceEventResponse>
    {
        ...
        public ResetInterfaceEventHandler(IAzureFunctionsClient azureFunctionClient, ICurrentUserService currentUserService, IApplicationDbContext dbContext, FullContext parDataFullContext, IMediator Mediator)
        {
        ...
        public async Task<ResetInterfaceEventResponse> Handle(ResetInterfaceEventCommand request, CancellationToken cancellationToken)
        {
            if (request.Id <= 0)
            {
            ...
            try
            {
                ...
                if (request.InterfaceType.ToLower().Contains("order"))
                {
                    var response = await _mediator.Send(new ResetOrderEventQueueEntryRequest() { Id = orderEvent.Id, Context = context }, cancellationToken);
                }
                else
                {
                    var response = await _mediator.Send(new ResetInventoryEventQueueEntryRequest() { Id = orderEvent.Id, Context = context }, cancellationToken);
                }
            ...
            return new ResetInterfaceEventResponse()
            {
                Id = request.Id
            };
        }
