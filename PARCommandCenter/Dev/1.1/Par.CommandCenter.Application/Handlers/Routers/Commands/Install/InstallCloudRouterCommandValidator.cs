...
{
    public class InstallCloudRouterCommandValidator : AbstractValidator<InstallCloudRouterCommand>
    {
        public InstallCloudRouterCommandValidator()
        {
            RuleFor(x => x.Address)
              .NotEmpty()
              .MaximumLength(14)
              .WithMessage("Router address must be not empty or exceed 14 Character.");
        ...
        private static bool HasFilterSet(InstallCloudRouterCommand criteria)
        {
            Debug.Assert(criteria != null, "criteria != null");

            if (!string.IsNullOrWhiteSpace(criteria.Address) && !string.IsNullOrWhiteSpace(criteria.ServiceName))
            {
